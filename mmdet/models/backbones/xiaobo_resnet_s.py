...
class softmaxattention1(nn.Module):
    ...
    def __init__(self,channel,dcn, k=1, s=1, p=None):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv5 = build_conv_layer(dcn, 2048, 4096, kernel_size=1, stride=1, padding=0, dilation=1, bias=False)
        self.cv6 = build_conv_layer(dcn, 8192, 2048, kernel_size=1, stride=1, padding=0, dilation=1, bias=False)
    ...
    def __init__(self,
        ...
        if self.conv2_stride==2:
            self.conv2 = build_conv_layer(
                ...
                dilation=dilation,
                bias=False)
            # self.switch = nn.Conv2d(planes, 1, kernel_size=1, stride=1, bias=True)
            self.xfm = DWTForward(J=1, wave='db1', mode='zero')
            # self.cv2 = build_conv_layer(
            #     dcn,
            #     planes*4,
            #     planes,
            #     kernel_size=3,
...
class xiaobo_ResNet_switchsimple(BaseModule):
    """ResNet backbone.
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(xiaobo_ResNet_switchsimple, self).__init__(init_cfg)
    ...
    def _make_stem_layer(self, in_channels, stem_channels):
        if self.deep_stem:
            ...
            self.relu = nn.ReLU(inplace=True)
        self.xfm = DWTForward(J=1, wave='db1', mode='zero')
        # self.switch = nn.Conv2d(64, 1, kernel_size=1, stride=1, bias=True)
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(xiaobo_ResNet_switchsimple, self).train(mode)
        self._freeze_stages()
...
@BACKBONES.register_module()
class xiaobo_ResNet_switchsimple1D(xiaobo_ResNet_switchsimple):
    ...
    def __init__(self, **kwargs):
        super(xiaobo_ResNet_switchsimple1D, self).__init__(
