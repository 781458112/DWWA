...
class LoadImageFromFile:
    ...
    def __call__(self, results):
        ...
        # print(filename)
        img_bytes = self.file_client.get(filename)   ##tupian

        img = mmcv.imfrombytes(img_bytes, flag=self.color_type)  ##tupian
        # filename = filename.split('/')[-1][:-4] + '.mat'  ##mat
        # img = scio.loadmat(osp.join(results['img_prefix'],filename))['finaldata']##mat
        # img=[(x - np.min(img)) / (np.max(img) - np.min(img))*255 for x in img ]# guiyi
        # img=np.transpose(img)
        # img = np.dstack((img, img,img))#mat
